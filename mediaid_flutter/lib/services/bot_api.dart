...
import 'package:mediaid_flutter/models/ai_model.dart';
import 'package:mediaid_flutter/models/chat_model.dart';
import 'package:mediaid_flutter/services/api_constant.dart';


class bot_API {
   static Future<List<AiModel>> getModels()async {
     try{
       var response = await http.get(Uri.parse("$gpt_URL/models"),
   ...
   // }
     try{
       var response = await http.post(Uri.parse("$gpt_URL/chat/completions"),
         headers: {
           'Authorization': 'Bearer $API_KEY',
           "Content-Type": "application/json"
         },
